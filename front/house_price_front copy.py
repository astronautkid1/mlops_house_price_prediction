...
if mode == "Formulaire manuel":
    ...
    gr_liv_area = st.number_input(
        "Surface habitable au-dessus du sol (GrLivArea)", min_value=0, value=1710
    )
    overall_qual = st.slider("Qualité globale (OverallQual)", 1, 10, value=7)
    year_built = st.number_input(
        "Année de construction (YearBuilt)", min_value=1800, max_value=2025, value=2003
    )
    total_bsmt_sf = st.number_input(
        "Surface sous-sol (TotalBsmtSF)", min_value=0, value=856
    )
    garage_area = st.number_input(
        "Surface garage  (GarageArea)", min_value=0, value=548
    )
    ...
    fireplace_qu = st.selectbox(
        "Qualité cheminée  (FireplaceQu)", ["Ex", "Gd", "TA", "Fa", "Po", None]
    )
    garage_type = st.selectbox(
        "GarageType",
        ["Attchd", "Detchd", "BuiltIn", "Basment", "CarPort", "2Types", None],
    )
...
# ------------------------- Mode CSV -------------------------
    ...
    uploaded_file = st.file_uploader(
        "Uploader un fichier CSV avec les caractéristiques des maisons", type=["csv"]
    )
